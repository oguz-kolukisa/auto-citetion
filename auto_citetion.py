...
def load_cookie(output_dir: Path) -> str:
    for path in [output_dir / ".scholar_inbox_cookie",
                 Path(__file__).parent / ".scholar_inbox_cookie"]:
        if path.exists():
...
def generate_report(papers: list[Paper]) -> str:
    ...
    lines += _build_topic_sections(papers)
    return "\n".join(lines)
...
def _build_topic_sections(papers: list[Paper]) -> list[str]:
...
def run_report_stage(papers, out) -> None:
    print("\n" + "=" * 50, "\nSTAGE 3: Report\n" + "=" * 50, file=sys.stderr)
